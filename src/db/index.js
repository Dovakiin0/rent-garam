...
pool.query(
  "CREATE TABLE IF NOT EXISTS estate (id SERIAL PRIMARY KEY, name VARCHAR(255),address VARCHAR(255), image_url VARCHAR(255), description VARCHAR(1000), bedroom INTEGER, washroom INTEGER, price FLOAT, owner_id INTEGER, latitude FLOAT, longitude FLOAT, type VARCHAR(255),createdAt TIMESTAMP DEFAULT NOW(),sold BOOLEAN, FOREIGN KEY (owner_id) REFERENCES users(id))"
);
pool.query(
